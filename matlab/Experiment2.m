...
% Load image
im1 = im2double(imread(imagename));

%Crope image
a=70;
b=96;
im=im1(a+1:a+128,b+1:b+128);
...
Mod = sopt_struct2size(Cv);

Psit3 = @(x) sopt_fwdcurvelet(x,realv); 
Psi3 = @(x) sopt_adjcurvelet(x,Mod,realv);

%% Gaussian matrix operator

num_meas=floor(p*numel(im));

Phi = randn(num_meas,numel(im))/sqrt(num_meas);
        
bound=svds(Phi,1)^2;
     
A = @(x) Phi*x(:);
    
At = @(x) reshape(Phi'*x(:),size(im));
...
param.nu_B2 = bound; % Bound on the norm of the operator A
...
sol1 = sopt_dr_BPDN(y, epsilon, A, At, Psi, Psit, param);
...
sol2 = sopt_dr_rwBPDN(y, epsilon, A, At, Psi, Psit, param, sigma, tol, maxiter, sol1);
...
sol4 = sopt_dr_rwBPDN(y, epsilon, A, At, Psi2, Psit2, param, sigma, tol, maxiter, sol3);
...
param3.nu_B2 = bound; % Bound on the norm of the operator A
...
sol5 = sopt_dr_BPDN(y, epsilon, A, At, Psi3, Psit3, param3);
...
sol6 = sopt_dr_rwBPDN(y, epsilon, A, At, Psi3, Psit3, param3, sigma, tol, maxiter, sol5);
...
param1.nu_B2 = bound; % Bound on the norm of the operator A
...
sol7 = sopt_dr_TVDN(y, epsilon, A, At, param1);
...
sol8 = sopt_dr_rwTVDN(y, epsilon, A, At, param1,sigma, tol, maxiter, sol7);
