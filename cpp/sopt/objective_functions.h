...
template <class T>
std::function<t_real(T)> const
unconstrained_regularisation(const std::function<t_real(T)> &g, const t_real &sig, const T &y,
                             const sopt::OperatorFunction<T> &measurement_operator);
//! returns ||ψ^t x||_1 + ||y - Φ x||_2^2 as a function
template <class T>
std::function<t_real(T)> const
unconstrained_L1_regularisation(const t_real &gamma, const t_real &sig, const T &y,
                                const sopt::OperatorFunction<T> &measurement_operator,
...
unconstrained_l1_regularisation(const t_real &gamma, const t_real &sig, const T &y,
                                ...
                                const sopt::LinearTransform<T> &wavelet_operator) {
  if(gamma <= 0)
    SOPT_THROW("regularsiation paramter is not greater than 0.");
  const std::function<t_real(T)> g
      = [=](const T &x) -> t_real { return sopt::l1_norm(wavelet_operator.adjoint() * x) * gamma; };
