 ...
 public:
  ...
  ImagingPrimalDual(Eigen::MatrixBase<DERIVED> const &target)
      : l2_proximal_([](t_Vector &out, const Real &gamma, const t_Vector &x) {
        ...
        l2_proximal_weighted_([](t_Vector &out, const Vector<Real> &gamma, const t_Vector &x) {
          proximal::l2_norm(out, gamma, x);
        }),
...
// Forwards get/setters to L2 and L2Ball proximals
...
#undef SOPT_MACRO
 ...
 protected:
  ...
  bool check_l2_weight_proximal(const t_Proximal<Real> &no_weights,
                                const t_Proximal<Vector<Real>> &with_weights) const {
    Vector<SCALAR> output;
    Vector<SCALAR> outputw;

    const Vector<SCALAR> x = Vector<SCALAR>::Ones(this->l2_proximal_weights().size());
...
typename ImagingPrimalDual<SCALAR>::Diagnostic ImagingPrimalDual<SCALAR>::operator()(
  ...
  if (not check_l2_weight_proximal(l2_proximal(), l2_proximal_weighted()))
    SOPT_THROW(
