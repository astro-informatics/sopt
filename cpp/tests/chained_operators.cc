...
TEST_CASE("Linear Transforms", "[ops]") {
  ...
  SECTION("linear transform") {
    ...
    OperatorFunction<t_Vector> const func0 = [](t_Vector &out, t_Vector const &input) { out = input.head(N - 1).array() * 2; };
    OperatorFunction<t_Vector> const func1 = [](t_Vector &out, t_Vector const &input) { out = input.head(N - 1).array() * 4; };
    OperatorFunction<t_Vector> const afunc0 = [](t_Vector &out, t_Vector const &input) { out = t_Vector::Zero(N); out.head(N - 1) = input.array() * 2; };
    OperatorFunction<t_Vector> const afunc1 = [](t_Vector &out, t_Vector const &input) { out = t_Vector::Zero(N); out.head(N - 1) = input.array() * 4; };
    t_Vector const x = t_Vector::Random(2 * N) * 5;
    auto chain = chained_operators(func0, func1, func0, func0);
