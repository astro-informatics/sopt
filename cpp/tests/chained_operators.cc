...
TEST_CASE("Linear Transforms", "[ops]") {
  ...
  SECTION("1 Functions") {
    ...
    OperatorFunction<t_Vector> const func0
        = [](t_Vector &out, t_Vector const &input) { out = input.array() * 2 - 1; };
    OperatorFunction<t_Vector> const func1
        = [](t_Vector &out, t_Vector const &input) { out = input.array() * 4 - 1; };
  ...
  SECTION("linear transform") {
    ...
    OperatorFunction<t_Vector> const func0
        = [](t_Vector &out, t_Vector const &input) { out = input.head(N - 1).array() * 2; };
    OperatorFunction<t_Vector> const func1
        = [](t_Vector &out, t_Vector const &input) { out = input.head(N - 1).array() * 4; };
